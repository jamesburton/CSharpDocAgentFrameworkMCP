...
- [ ] **CORE-01**: Stable `SymbolId` spec with assembly-qualified identity and rename tracking (`PreviousIds`)
...
- [ ] **INDX-01**: BM25 search index over symbol names and doc text replacing `InMemorySearchIndex`
- [ ] **INDX-02**: CamelCase-aware tokenization for symbol name search
- [ ] **INDX-03**: Index persistence alongside snapshots

### Query

- [ ] **QURY-01**: `IKnowledgeQueryService` facade wired to `ISearchIndex` + `SnapshotStore`
- [ ] **QURY-02**: `SearchAsync` — ranked symbol search results
- [ ] **QURY-03**: `GetSymbolAsync` — full symbol detail by ID
